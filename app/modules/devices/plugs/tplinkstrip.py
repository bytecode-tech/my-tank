...
class TplinkStrip(Strip):
    ...
    def toggle(self, *, index: int = -1):
        """Toggle device state"""
        if index < 0:
            for x in range(self.native_api.num_children):
                self.native_api.turn_on(index=x) if self.native_api.get_is_off(index=x) else self.native_api.turn_off(index=x)
        else:
